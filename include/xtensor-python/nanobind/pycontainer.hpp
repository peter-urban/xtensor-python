...
namespace xt
{
    namespace nanobind
    {
        ...
        /**
         * Reshapes the container in-place without copying data.
         * @param shape the new shape
         * @param layout the layout to use for computing new strides
         * @return reference to the reshaped container
         * 
         * Note: Unlike pybind11 which can use PyArray_Newshape for in-place reshape,
         * nanobind requires us to update the shape/strides metadata while keeping
         * the same underlying data buffer. This preserves the data pointer, matching
         * the behavior expected by xtensor and consistent with numpy's reshape.
         */
        template <class D>
        template <class S>
        inline auto& pycontainer<D>::reshape(S&& shape, layout_type layout) &
        {
            ...
            // Convert shape to our shape_type
            shape_type new_shape = xtl::forward_sequence<shape_type, S>(std::forward<S>(shape));
            
            // Compute new strides based on the layout
            strides_type new_strides = xtl::make_sequence<strides_type>(new_shape.size(), size_type(1));
            compute_strides(new_shape, layout, new_strides);
            
            // Get pointer to underlying data and total size
            auto data_ptr = this->data();
            auto total_size = this->size();
            
            // Call the derived class's reset_from_reshape method which updates
            // shape/strides without reallocating memory, preserving the data pointer
            derived_cast().reset_from_reshape(new_shape, new_strides, data_ptr, total_size);
